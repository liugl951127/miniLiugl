...
#   packages/daemon/skills/docx/scripts/setup.sh
# Adds a CLOUD_SANDBOX env-gate at the top of main() so apt/brew install
# steps are skipped (deps baked into cloud-compute-runtime image).
...
main() {
  # ── CLOUD-OVERLAY: cloud sandbox env-gate ─────────────────────────────
  # Cloud sandbox bakes .NET 9 / pandoc / soffice / poppler / zip into the
  # cloud-compute-runtime image. Re-running install_* there would WARN on
  # missing apt sudo and waste minutes; skip straight to build + verify.
  if [ "${CLOUD_SANDBOX:-0}" = "1" ] || [ -f /.dockerenv ]; then
    echo "============================================"
    echo "  docx Setup (cloud sandbox mode)"
    echo "  $(date "+%Y-%m-%d %H:%M:%S")"
    echo "============================================"
    : > "$LOG_FILE"
    detect_platform
    log "cloud sandbox detected — skipping apt/brew installs (deps baked into image)"
    check_locale
    check_nuget_config
    fix_permissions
    build_project
