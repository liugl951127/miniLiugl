...
### Added
- **本地自研模型集成**: OnnxLLMAdapter + MiniTransformer.generate() 自回归推理
  - protocol=onnx → 直接加载本地 .onnx 模型文件，不依赖 Ollama/vLLM
  - 流式 + 阻塞双模式，模型缓存避免重复加载
  - 支持 temperature / top-p nucleus 采样
- **SSRF 防护修复 (LocalModelController)**: 改为允许 localhost/私有内网（本地模型服务器必需）
- **本地模型前端分组**: chat/Index.vue 模型选择器新增「💻 本地模型」独立分组
- **前端模型列表修复**: loadModels() 改为调 /models（含本地模型），修复 provider→models 路径

### Security (V6.8.2)
- **JWT Secret 硬化**: 移除 yml 中所有 `${MINIMAX_JWT_SECRET:默认值}` → `${MINIMAX_JWT_SECRET}` 无默认值
- **凭证硬化**: Redis/Nacos/MinIO 密码全部移除默认值，强制环境变量注入
- **RAG IDOR 修复**: /retrieve /ask /doc /doc/{id}/chunks 全部加 verifyAccess() 归属校验
- **审计授权修复**: AuditController 加 @PreAuthorize("hasRole('ADMIN')") + @EnableMethodSecurity
- **CORS 统一**: 12 SecurityConfig 全部引用 CorsUtils.buildCorsConfigurationSource()
- **8个模块 import 修复**: agent/chat/monitor/multimodal/pipeline/rag 等补 CorsConfigurationSource import

### Fixed
- AuditController.byDay(): Math.random() 假数据 → selectCount 真实 DB 统计
- 前端 /models 接口修复: 调 /api/v1/models（返回模型列表）而非 /model/providers/page（返回服务商）

### Infrastructure
- **Docker 凭证硬化**: docker-compose.yml x-jvm-env 移除所有默认值，`${VAR?必填}` 强制必填
- **.env.example**: 创建环境变量示例文件（.env 已在 .gitignore）
- **README-DOCKER.md V6.8.2**: 更新部署说明，必填环境变量表格

## [6.8.10] - 2026-08-10

### Fixed
- 登录后 401 "缺少 token" 根因: 29 个 view 直接调用 `http.get('admin/xxx')` 无前导 `/`，http.js interceptor 不追加 `/api/v1` 前缀，导致请求到 auth 服务报错
  - 修复: 批量给 40+ 处 API 调用加 `/` 前缀
  - 影响: api/admin.js + 11 个 V2 view + knowledge/memory view
- CrudTable.vue / FormDrawer.vue reactive `.value` 模板错 (7 处)

### V6.8.9 - 2026-08-09

### Fixed
- 前端全量静态审计: 29 个 view t() import 100% OK
- 菜单路径 29/29 对齐
- 4 菜单点击无法跳转: `/ai/ai-ppt` → `/ai-ppt`
- `t.t is not a function`: layout/Index.vue + 4 view 补 t import
- schema 缺 5 列: `ALTER TABLE ai_chat_session ADD COLUMN status ...`

### Refactored
- 批量重构剩余 24 view (ai 10 + analytics 4 + pipeline 1...)
- 删除 39 个旧 view (节省 14388 行)
- V2 路由全部指向新组件

### V6.8.5 ~ V6.8.8 - 2026-08-08

### Fixed
- 12 V2 view `import { http }` → default import
- 9 speechCall `.value` 模板错
- `/login` 路由链路 100% 对齐
- API 路径 100% 对齐 (130 backend vs 441 frontend)
- build 0 错 0 警 / vitest 269 通过 / E2E 5 端点全通

### Refactored
- 17 admin 路由切 V2
- 25 单元测试 92.6% 通过
- PageStandard/CrudTable/FormDrawer 业务组件化
