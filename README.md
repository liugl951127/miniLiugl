...
> **V6.8.2** · 14 个微服务模块 · Spring Cloud Gateway · Nacos 服务发现 · Prometheus 全链路监控 · TraceId 全链路追踪 · GitHub Actions CI · 前端 40+ 页面 · **Docker Compose 一键部署** · 80+ 张数据表 · **@Audited AOP 审计** · **Skill 审批流** · **DLP 脱敏** · **限流 (Redis Token Bucket)**
>
...
| **限流** | Spring Cloud Gateway `RequestRateLimiter` + Redis Token Bucket + 按 API Key/IP/User 三维度 |
| **合规审计** | `@Audited` AOP 注解 + `@Async` 异步入库 + DLP 脱敏 (手机号/身份证/银行卡/JWT) |
| **Skill 审批流** | HIGH/CRITICAL 工具自动创建 PENDING → 管理员 approve/reject → 执行或拒绝 |
| **Docker Compose** | 16 服务一键部署 (13 微服务 + MariaDB + Redis + Nacos + nginx) |
