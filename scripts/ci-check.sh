...
BAD1=$(grep -rn "NOT NULL DEFAULT ''" sql/*.sql 2>/dev/null | \
       grep -E "(INT|BIGINT|TIMESTAMP) NOT NULL DEFAULT ''" | head -5)
if [ -n "$BAD1" ]; then
...
BAD2=$(grep -rn "jdbc:mysql://" backend/minimax-*/src/main/resources/application*.yml 2>/dev/null | head -5)
if [ -n "$BAD2" ]; then
...
BAD3=$(grep -rn -E "com\.mysql\.(cj\.)?jdbc\.Driver" backend/minimax-*/src/main/resources/application*.yml 2>/dev/null | head -5)
if [ -n "$BAD3" ]; then
