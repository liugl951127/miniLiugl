# MiniMax Platform V6.8.2 — Docker 一键部署
...
| 变量 | 说明 | 示例 |
|------|------|------|
| `MINIMAX_JWT_SECRET` | JWT 签名密钥 (64字符 hex) | `openssl rand -hex 32` 生成 |
| `MINIMAX_COMMON_SECRET` | 通用加密密钥 | 同上 |
| `REDIS_PASS` | Redis 密码 | `your_redis_password` |
| `NACOS_USER` | Nacos 用户名 | `nacos` |
| `NACOS_PASS` | Nacos 密码 | `your_nacos_password` |
| `MYSQL_ROOT_PASSWORD` | MariaDB root 密码 | `your_mysql_password` |
| `CORS_ORIGINS` | 允许的跨域域名 | `https://your.domain.com` |

**生成随机密钥:**
```bash
openssl rand -hex 32
# 例: 8f3a2b1c9e4d5f6a7b8c9d0e1f2a3b4c...
```
