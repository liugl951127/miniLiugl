-- ============================================================
-- MiniMax Platform - 全量初始化 SQL
-- ============================================================
-- 一个文件搞定所有表 + 数据:
--   - 14 个模块 (auth/chat/model/memory/rag/function/admin/
--     multimodal/monitor/agent/prompt/ws/optimization/tenant)
--   - 33 张表
--   - 6 个 provider + 22 个 model_config
--   - 默认账号: adminLiugl / Liugl@2026 (密码由 AdminDataInitializer 启动时 BCrypt 写入)
--
-- 用法:
--   mysql -uroot < sql/minimax-all.sql
-- 或:
--   bash scripts/init-sql.sh
--
-- 生成时间: $(date)
...
-- ▸ 来源: sql/init/07_memory_long.sql
-- ============================================================
...
-- ▸ 来源: sql/init/09_function_calling.sql
-- ============================================================
...
-- ▸ 来源: sql/init/13_optimization.sql
-- ============================================================
...
-- ▸ 来源: sql/init/15_v2_features.sql
-- ============================================================
...
-- ▸ 来源: sql/init/16_super_admin.sql
-- ============================================================
...
-- ▸ 来源: sql/init/19_v4_3_prompt.sql
-- ============================================================
-- 19_v4_3_prompt.sql
...
-- ============================================================
-- V4 Mock Provider (沙箱演示用, 配 key 后可去掉)
-- ============================================================
INSERT IGNORE INTO `model_provider`
  (`code`, `name`, `base_url`, `api_key`, `protocol`, `enabled`, `sort`, `description`)
VALUES
  ('mock', 'Mock Adapter', 'http://localhost', 'mock', 'mock', 1, 99, '沙箱演示用, 无 key 也能跑');

INSERT IGNORE INTO `model_config`
  (`provider_id`, `model_code`, `display_name`, `max_context`, `max_output`,
   `supports_vision`, `supports_tools`, `supports_stream`, `enabled`, `sort`, `description`)
SELECT `id`, 'mock', 'Mock (沙箱演示)', 8000, 2048,
       0, 0, 1, 1, 99, '沙箱无 key 演示'
FROM `model_provider` WHERE `code` = 'mock';

-- ============================================================
-- 验证 (跑完后可手动执行)
-- ============================================================
-- SELECT COUNT(*) AS tables_count FROM information_schema.tables WHERE table_schema = 'minimax';
-- SELECT COUNT(*) AS providers FROM model_provider;
-- SELECT COUNT(*) AS models FROM model_config;
-- SELECT COUNT(*) AS prompt_templates FROM prompt_template;
