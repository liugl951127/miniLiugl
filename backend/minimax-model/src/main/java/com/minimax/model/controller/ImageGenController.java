...
public class ImageGenController {
    ...
    @Value("${minimax.imagegen.mock-mode:false}")
    ...
    public Result<List<Map<String, Object>>> models() {
        ...
        // 提示词优化模型 (不生成图片, 只优化 prompt)
        ms.add(Map.of(
                "code", "deepseek-v4-flash",
                "displayName", "DeepSeek V4 Flash (提示词优化)",
                "provider", "deepseek",
                "size", "prompt-only",
                "pricePerImg", 0,
                "enabled", true
        ...
        ms.add(Map.of(
                "code", "deepseek-chat",
                "displayName", "DeepSeek Chat (提示词优化)",
                "provider", "deepseek",
                "size", "prompt-only",
                "pricePerImg", 0,
                "enabled", true
    ...
    @Operation(summary = "文生图服务健康检查")
    @GetMapping("/health")
    public Result<Map<String, String>> health() {
        Map<String, String> m = new LinkedHashMap<>();
        m.put("siliconflow", System.getenv("SILICONFLOW_API_KEY") != null ? "ok" : "not_configured");
        m.put("dashscope",   System.getenv("DASHSCOPE_API_KEY")   != null ? "ok" : "not_configured");
        m.put("openai",     System.getenv("OPENAI_API_KEY")       != null ? "ok" : "not_configured");
        return Result.ok(m);
    }

    @Operation(summary = "估算生成费用")
    @PostMapping("/price-estimate")
    public Result<Map<String, Object>> priceEstimate(@RequestBody Map<String, Object> body) {
        ...
        Map<String, Double> priceMap = Map.ofEntries(
                Map.entry("black-forest-labs/FLUX.1-schnell", 0.0),
                Map.entry("stabilityai/stable-diffusion-xl-base-1.0", 0.0),
                Map.entry("Kwai-Kolors/Kolors", 0.0),
                Map.entry("wanx-v1", 0.0),
                Map.entry("dall-e-3", 0.04),
                Map.entry("deepseek-v4-flash", 0.0),
                Map.entry("deepseek-chat", 0.0)
        );
        double unitPrice = priceMap.getOrDefault(model, 0.0);
        double total = unitPrice * count;
        Map<String, Object> out = new LinkedHashMap<>();
        out.put("model", model);
        out.put("count", count);
        out.put("pricePerImg", unitPrice);
        out.put("totalUsd", total);
        out.put("note", total == 0 ? "免费或提示词优化模型" : String.format("%.4f USD / 张", unitPrice));
