...
public class TrainingController {
  ...
  @Operation(summary = "训练历史指标 (loss 曲线)")
  public Result<Map<String, Object>> history(@PathVariable Long id) {
    Map<String, Object> h = trainingService.getHistory(id);
    if (h.isEmpty()) return Result.fail("任务不存在: " + id);
    return Result.ok(h);
  }

  @GetMapping("/tasks/{id}/logs")
  @Operation(summary = "V6.8.1: 训练日志")
  public Result<List<Map<String, Object>>> logs(@PathVariable Long id) {
    return Result.ok(trainingService.getLogs(id));
  }
}
