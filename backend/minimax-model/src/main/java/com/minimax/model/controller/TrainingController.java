...
public class TrainingController {
  ...
  public Result listModels() {
    return Result.success(trainingService.listModels());
  }

  @PostMapping("/tasks")
  @Operation(summary = "创建训练任务")
  public Result createTask(@RequestBody TrainingTaskDTO dto, @AuthenticatedUser Long userId) {
  ...
  public Result listTasks(@AuthenticatedUser Long userId) {
    return Result.success(trainingService.listByUser(userId));
  }
  ...
  public Result getTask(@PathVariable Long id) {
    ...
    return t != null ? Result.success(t) : Result.fail("任务不存在");
  }
  ...
  public Result cancel(@PathVariable Long id) {
    ...
    return ok ? Result.success(null) : Result.fail("无法取消(任务不存在或已完成)");
  }
}
