...
public class ProviderController {
    ...
    // V6.8.1: 启用/禁用 Provider (model/Index.vue toggleModel 调用)
    // 支持按 id (Long) 或 code (String) 查找
    @Operation(summary = "启用/禁用 Provider (V6.8.1)")
    @PostMapping("/toggle")
    public Result<Map<String, Object>> toggle(@RequestParam(required = false) Long id,
                                               @RequestParam(required = false) String code) {
        ModelProvider p = null;
        if (id != null) {
            p = mapper.selectById(id);
        } else if (code != null) {
            p = mapper.selectOne(new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<ModelProvider>()
                    .eq(ModelProvider::getCode, code));
        } else {
            return Result.fail(400, "id 或 code 必须提供一个");
        }
        if (p == null) return Result.fail(404, "provider not found");
        int newStatus = (p.getEnabled() == null || p.getEnabled() == 0) ? 1 : 0;
        p.setEnabled(newStatus);
        mapper.updateById(p);
        return Result.ok(Map.of("enabled", newStatus, "id", p.getId()));
    }
}
