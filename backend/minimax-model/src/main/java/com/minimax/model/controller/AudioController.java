...
public class AudioController {
    ...
    @Value("${minimax.audio.mock-mode:false}")
    ...
    public Result<List<Map<String, Object>>> asrModels() {
        return Result.ok(List.of(
                ...
                Map.of("code", "FunAudioLLM/SenseVoiceSmall", "displayName", "SenseVoice Small (硅基流动)",
                        "provider", "siliconflow", "languages", "中英日韩粤", "enabled", true),
                Map.of("code", "mock", "displayName", "Mock (沙箱演示, 返回固定文字)",
                        "provider", "mock", "languages", "中文", "enabled", true)
        ));
    }

    @Operation(summary = "音频服务健康检查")
    @GetMapping("/health")
    public Result<Map<String, String>> health() {
        String apiKey = System.getenv("SILICONFLOW_API_KEY");
        String status = (apiKey != null && !apiKey.isBlank()) ? "ok" : "not_configured";
        return Result.ok(Map.of("asr", status, "tts", status));
    }
    ...
    public Result<Map<String, Object>> transcribe(
        ...
        try {
            ...
            if (apiKey == null || apiKey.isBlank()) {
                return Result.fail(500, "SILICONFLOW_API_KEY 未配置. 请设置环境变量 SILICONFLOW_API_KEY (硅基流动 key) 后重试. 申请地址: https://cloud.siliconflow.cn");
            }
    ...
    public Result<List<Map<String, Object>>> ttsVoices() {
        String apiKey = System.getenv("SILICONFLOW_API_KEY");
        if (apiKey != null && !apiKey.isBlank()) {
            // 尝试从 SiliconFlow CosyVoice2 拉取真实音色列表
            try {
                HttpRequest req = HttpRequest.newBuilder()
                        .uri(URI.create("https://api.siliconflow.cn/v1/audio/voices"))
                ...
                if (resp.statusCode() == 200) {
                    ...
                    @SuppressWarnings("unchecked")
                    List<Map<String, Object>> voices = (List<Map<String, Object>>) body.getOrDefault("data", List.of());
                    List<Map<String, Object>> result = new ArrayList<>();
                    for (Map<String, Object> v : voices) {
                        Map<String, Object> m = new LinkedHashMap<>();
                        m.put("code", v.getOrDefault("voice_id", v.getOrDefault("id", "")));
                        m.put("name", v.getOrDefault("name", ""));
                        m.put("language", v.getOrDefault("language", ""));
                        m.put("gender", v.getOrDefault("gender", ""));
                        m.put("provider", "siliconflow");
                        result.add(m);
                    }
                    if (!result.isEmpty()) return Result.ok(result);
                }
            } catch (Exception e) {
                log.warn("从 SiliconFlow 拉取音色列表失败: {}", e.getMessage());
            }
        }
        // 回退: 硬编码 SiliconFlow CosyVoice2-0.5B 音色
        return Result.ok(List.of(
                Map.of("code", "smile",         "name", "晓晓 (温柔女声)",    "language", "zh-CN", "gender", "Female", "provider", "siliconflow"),
                Map.of("code", "songlaoshi",    "name", "云希 (青年男声)",    "language", "zh-CN", "gender", "Male",   "provider", "siliconflow"),
                Map.of("code", "yiyi",          "name", "一一 (活泼女声)",    "language", "zh-CN", "gender", "Female", "provider", "siliconflow"),
                Map.of("code", "tianke",        "name", "天珂 (磁性男声)",    "language", "zh-CN", "gender", "Male",   "provider", "siliconflow"),
                Map.of("code", "ninger",        "name", "宁儿 (知性女声)",    "language", "zh-CN", "gender", "Female", "provider", "siliconflow"),
                Map.of("code", "xiaona",        "name", "小娜 (甜美女声)",    "language", "zh-CN", "gender", "Female", "provider", "siliconflow"),
                Map.of("code", "zhizhi",        "name", "知知 (自然女声)",    "language", "zh-CN", "gender", "Female", "provider", "siliconflow"),
                Map.of("code", "mock",          "name", "Mock (沙箱演示)",    "language", "zh",    "gender", "Neutral", "provider", "mock")
        ));
    }
