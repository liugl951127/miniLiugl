...
@PreAuthorize("hasRole('ADMIN')")  // V6.8.2: 仅管理员可访问审计日志
...
public class AuditController {
    ...
    public Result<List<Map<String, Object>>> byDay(@RequestParam(defaultValue = "7") int days) {
        ...
        for (int i = days - 1; i >= 0; i--) {
            ...
            // 统计总数
            QueryWrapper<AuditLogFull> totalQ = new QueryWrapper<>();
            totalQ.between("created_at", dayStart, dayEnd);
            long total = auditMapper.selectCount(totalQ);
            // 统计失败数
            QueryWrapper<AuditLogFull> failQ = new QueryWrapper<>();
            failQ.between("created_at", dayStart, dayEnd).eq("result", "FAIL");
            long failures = auditMapper.selectCount(failQ);
            Map<String, Object> row = new LinkedHashMap<>();
            row.put("day", dayStart.toLocalDate().toString());
            row.put("count", total);
            row.put("success", total - failures);
            row.put("failure", failures);
            result.add(row);
        }
