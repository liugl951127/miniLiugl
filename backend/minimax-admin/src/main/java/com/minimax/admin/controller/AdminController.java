...
public class AdminController {
    ...
    // V6.8.2: 写接口全部要求 ADMIN 角色，不再信任 actorId 请求参数
    @Operation(summary = "创建用户 (V6.8.2 需ADMIN)")
    @PostMapping("/users")
    @PreAuthorize("hasRole('ADMIN')")
    public Result<String> createUser(@AuthenticationPrincipal AuthenticatedUser user,
                                      @RequestBody Map<String, Object> body,
                                      HttpServletRequest req) {
        return Result.ok(userMgmt.createUser(user.id(), user.username(), body, req));
    }

    @Operation(summary = "重置用户密码 (V6.8.2 需ADMIN)")
    ...
    public Result<String> resetPassword(@PathVariable("id") Long userId,
                                          ...
                                          HttpServletRequest req) {
        return Result.ok(userMgmt.resetPassword(user.id(), user.username(), userId, body.get("newPassword"), req));
    }

    @Operation(summary = "启用/禁用用户 (V6.8.2 需ADMIN)")
    ...
    public Result<String> toggleUser(@PathVariable("id") Long userId,
                                      ...
                                      HttpServletRequest req) {
        return Result.ok(userMgmt.toggleUser(user.id(), user.username(), userId, enabled, req));
    }
    ...
    @Operation(summary = "更新模型限流配置 (V6.8.2 需ADMIN)")
    ...
    public Result<String> updateRateLimit(@PathVariable("code") String code,
        ...
        return Result.ok(modelMgmt.updateRateLimit(user.id(), user.username(), code, capacity, refill, req));
    }
