...
public interface CollabParticipantMapper extends BaseMapper<CollabParticipant> {
     ...
     * 查询房间内所有参与者 (包括离线)
     */
    @Select("SELECT * FROM collab_participant WHERE roomId = #{roomId} ORDER BY joinedAt ASC")
    List<CollabParticipant> findByRoomId(@Param("roomId") String roomId);

    /**
     * 查询房间内在线参与者
     */
    @Select("SELECT * FROM collab_participant WHERE roomId = #{roomId} " +
     ...
     * 用户在该房间的参与记录
     */
    @Select("SELECT * FROM collab_participant WHERE roomId = #{roomId} AND userId = #{userId} " +
     ...
     * 标记用户离开
     */
    @Update("UPDATE collab_participant SET status = 'OFFLINE', leftAt = NOW() " +
     ...
     * 更新心跳
     */
    @Update("UPDATE collab_participant SET lastHeartbeat = NOW() " +
            "WHERE roomId = #{roomId} AND userId = #{userId} AND leftAt IS NULL")
     ...
     * 更新光标位置
     */
    @Update("UPDATE collab_participant SET cursorX = #{x}, cursorY = #{y}, selectionId = #{selectionId}, " +
            "lastHeartbeat = NOW() " +
            "WHERE roomId = #{roomId} AND userId = #{userId} AND leftAt IS NULL")
