...
public interface CollabMessageMapper extends BaseMapper<CollabMessage> {
     ...
     * 查询房间最近消息 (按时间正序, 用于回放)
     */
    @Select("SELECT * FROM collab_message WHERE room_id = #{roomId} " +
     ...
     */
    @Select("SELECT * FROM collab_message WHERE room_id = #{roomId} " +
            "AND type IN ('CHAT', 'AI') ORDER BY created_at DESC LIMIT #{limit}")
    List<CollabMessage> findChatHistory(@Param("roomId") String roomId, @Param("limit") int limit);

    /**
     * 统计房间消息数
     */
    @Select("SELECT COUNT(*) FROM collab_message WHERE room_id = #{roomId}")
