...
public interface AgentRatingMapper extends BaseMapper<AgentRating> {
    ...
    @Select("SELECT * FROM agent_rating WHERE agent_key = #{agentKey} ORDER BY created_at DESC LIMIT #{limit}")
    List<AgentRating> findByAgentKey(@Param("agentKey") String agentKey, @Param("limit") int limit);

    @Select("SELECT * FROM agent_rating WHERE agent_key = #{agentKey} AND user_id = #{userId} LIMIT 1")
