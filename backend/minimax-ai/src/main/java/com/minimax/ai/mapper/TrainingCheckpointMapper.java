...
public interface TrainingCheckpointMapper extends BaseMapper<TrainingCheckpoint> {
    ...
    @Select("SELECT * FROM training_checkpoint WHERE task_id = #{taskId} ORDER BY epoch DESC, step DESC")
    List<TrainingCheckpoint> findByTaskId(@Param("taskId") String taskId);

    /** 按 taskId + tag 查 (e.g. "best", "latest") */
    @Select("SELECT * FROM training_checkpoint WHERE task_id = #{taskId} AND tags LIKE CONCAT('%', #{tag}, '%') " +
    ...
    @Select("SELECT * FROM training_checkpoint WHERE task_id = #{taskId} ORDER BY val_loss ASC LIMIT 1")
