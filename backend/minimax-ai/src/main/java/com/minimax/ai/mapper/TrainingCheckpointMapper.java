...
public interface TrainingCheckpointMapper extends BaseMapper<TrainingCheckpoint> {
    ...
    @Select("SELECT * FROM training_checkpoint WHERE taskId = #{taskId} ORDER BY epoch DESC, step DESC")
    List<TrainingCheckpoint> findByTaskId(@Param("taskId") String taskId);

    /** 按 taskId + tag 查 (e.g. "best", "latest") */
    @Select("SELECT * FROM training_checkpoint WHERE taskId = #{taskId} AND tags LIKE CONCAT('%', #{tag}, '%') " +
    ...
    @Select("SELECT * FROM training_checkpoint WHERE taskId = #{taskId} ORDER BY valLoss ASC LIMIT 1")
    TrainingCheckpoint findBestByTaskId(@Param("taskId") String taskId);

    /** 按 checkpointId 查 */
    @Select("SELECT * FROM training_checkpoint WHERE checkpointId = #{checkpointId} LIMIT 1")
