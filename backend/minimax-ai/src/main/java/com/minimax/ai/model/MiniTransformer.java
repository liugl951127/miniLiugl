...
public class MiniTransformer {
    ...
    public List<TransformerBlock> getLayers() { return Arrays.asList(blocks); }
     ...
     *
     * @param prompt       输入提示词
     * @param temperature 采样温度 (0=贪心, >0 越有创意)
     * @param maxTokens    最大生成 token 数
     * @param topP         nucleus 采样阈值 (0~1)
     * @return GeneratedResult 包含生成文本、是否遇 EOS、token 统计
     */
    public GeneratedResult generate(String prompt, double temperature, int maxTokens, double topP) {
        ...
        for (int step = 0; step < maxTokens && pos < ctxLen; step++) {
            // 截断输入
            int[] input = new int[pos];
            System.arraycopy(tokens, 0, input, 0, pos);
            ...
            // 温度采样
            if (temperature > 0) {
                for (int v = 0; v < lastLogits.length; v++) lastLogits[v] /= temperature;
            }
            // softmax
            double max = Double.NEGATIVE_INFINITY;
            for (double d : lastLogits) if (d > max) max = d;
            double sum = 0;
            double[] probs = new double[lastLogits.length];
            for (int v = 0; v < lastLogits.length; v++) {
                probs[v] = Math.exp(lastLogits[v] - max);
                sum += probs[v];
            }
            ...
            for (int v = 0; v < probs.length; v++) {
                if (topP < 1.0 && cumsum > topP) { cutoff = v; break; }
                cumsum += probs[v];
            }
