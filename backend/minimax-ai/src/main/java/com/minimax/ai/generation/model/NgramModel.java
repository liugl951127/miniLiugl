...
@Slf4j
/**
 * NgramModel (V6.1 详细注释版)
 *
 * <h2>职责</h2>
 * ML 模型 - NgramModel.java
 *
 * <h2>核心功能</h2>
 * <ul>
 *   <li>提供 NgramModel 的业务能力</li>
 *   <li>参与 AI 平台整体架构</li>
...
public class NgramModel {
    ...
    /**
     * V6.5+ 重新加载缓存
     * 从 MySQL 重新拉取 ai_intent_keyword, 重建 n-gram 频率表
     */
    public void reload() {
        log.info("[NgramModel] V6.5+ 重新加载...");
        // 触发 @PostConstruct 重新执行
        try {
            java.lang.reflect.Method m = this.getClass().getDeclaredMethod("init");
            m.setAccessible(true);
            m.invoke(this);
            log.info("[NgramModel] 重新加载完成");
        } catch (Exception e) {
            log.warn("[NgramModel] 重新加载失败: {}", e.getMessage());
        }
    }
}
