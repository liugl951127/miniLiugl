...
public interface TrainingMetricMapper extends BaseMapper<TrainingMetric> {
    ...
    @Select("SELECT * FROM training_metric WHERE task_id = #{taskId} ORDER BY step ASC")
    List<TrainingMetric> findByTaskId(@Param("taskId") String taskId);

    /** 按 taskId 查最近 N 条 */
    @Select("SELECT * FROM training_metric WHERE task_id = #{taskId} ORDER BY step DESC LIMIT #{limit}")
    List<TrainingMetric> findRecent(@Param("taskId") String taskId, @Param("limit") int limit);

    /** 计数 */
    @Select("SELECT COUNT(*) FROM training_metric WHERE task_id = #{taskId}")
