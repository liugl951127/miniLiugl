...
public class AgentService {
    ...
    public AgentResult run(Long userId, String goal, List<String> tools) {
        ...
        // V6.8.1: 生成任务 ID 用于审批追踪
        String taskId = "agent-" + Math.abs((goal + System.currentTimeMillis()).hashCode());
        ...
        for (int round = 1; round <= maxRounds; round++) {
            ...
            for (var tc : resp.toolCalls) {
                ...
                try {
                    ...
                    // V6.8.1: HIGH/CRITICAL 工具审批检查
                    String checkResult = checkApproval(tc.function.name, userId, taskId, goal, argsMap);
                    if (checkResult != null) {
                        step.observation = checkResult;
                        step.durationMs = System.currentTimeMillis() - ts;
                        return AgentResult.fail(
                                "PENDING_APPROVAL: " + checkResult,
                                steps, round, toolsUsed
                        );
                    }
    ...
    private String checkApproval(String toolName, Long userId, String taskId, String goal, Map<String, Object> argsMap) {
        ...
        return String.format(
                "TOOL[%s] RISK[%s] 审批中(id=%d)。请等待管理员审批。",
                toolName, riskLevel, latest.getId()
        );
    }
