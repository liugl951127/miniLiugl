 ...
 * V2.8.7 实时协作 API - minimax-ws (8095)
 */
// V2.8.7 实时协作 API
import http from './http'

// ===== 房间 CRUD =====
export const createRoom = (data) => http.post('/api/v1/collab/rooms', data)
export const getRoom = (roomId) => http.get(`/api/v1/collab/rooms/${roomId}`)
export const listPublicRooms = (limit = 50) => http.get('/api/v1/collab/rooms/public', { params: { limit } })
export const closeRoom = (roomId, userId) => http.delete(`/api/v1/collab/rooms/${roomId}`, { params: { userId } })

// ===== 参与者 / 消息 =====
export const getParticipants = (roomId, onlineOnly = true) =>
  http.get(`/api/v1/collab/rooms/${roomId}/participants`, { params: { onlineOnly } })
...
export function buildCollabWsUrl(roomId, user) {
  ...
  // dev: Vite proxy; prod: nginx 代理到 8095
