...
export const deleteLongTerm = (id, userId) =>
  userId ? http.delete(`/memory/long-term/${id}?userId=${userId}`) : http.delete(`/memory/long-term/${id}`)

// 偏好
export const setPref = (userId, key, value, source) =>
  userId ? http.put(`/memory/pref/${key}?userId=${userId}`, { value, source }) : http.put(`/memory/pref/${key}`, { value, source })

export const getPref = (userId, key) =>
  userId ? http.get(`/memory/pref/${key}?userId=${userId}`) : http.get(`/memory/pref/${key}`)
