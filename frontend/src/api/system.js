...
export const systemApi = {
   ...
   * @returns GET /api/v1/monitor/health 的响应 Promise
   */
  healthAll: () => http.get('/monitor/health')
}
...
export const sessionApi = {
   ...
   * @returns GET  的响应 Promise
   */
  messages: (id, params) => http.get('/sessions/${id}/messages`, { params }),
   ...
   * @returns GET  的响应 Promise
   */
  streamStatus: (streamId) => http.get('/sessions/stream-status/${streamId}`),
  /** 停止流 */
  /**
   * stopStream - 创建/更新 
   * @returns POST  的响应 Promise
   */
  stopStream: (streamId) => http.post('/sessions/stop-stream?streamId=${streamId}`)
}
...
export const knowledgeApi = {
   ...
   */
  list: (params) => http.get('/rag/kb', { params }),
  /**
   * create - 创建/更新 /api/v1/rag/kb
   * @returns POST /api/v1/rag/kb 的响应 Promise
   */
  create: (data) => http.post('/rag/kb', data),
  /**
   * detail - 查询 
   * @returns GET  的响应 Promise
   */
  detail: (id) => http.get('/rag/kb/${id}`),
  /**
   * update - 替换 
   * @returns PUT  的响应 Promise
   */
  update: (id, data) => http.put('/rag/kb/${id}`, data),
  /**
   * remove - 删除 
   * @returns DELETE  的响应 Promise
   */
  remove: (id) => http.delete('/rag/kb/${id}`),
  /**
   * public - 查询 /api/v1/rag/kb/public
   * @returns GET /api/v1/rag/kb/public 的响应 Promise
   */
  public: () => http.get('/rag/kb/public')
}
