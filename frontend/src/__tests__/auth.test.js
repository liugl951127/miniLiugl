...
describe('Auth API', () => {
  ...
  describe('authApi.login', () => {
    it('应调用 POST /auth/login', async () => {
      ...
      expect(mockHttp.post).toHaveBeenCalledWith('/auth/login', { username: 'admin', password: 'password' })
    })
  ...
  describe('authApi.register', () => {
    it('应调用 POST /auth/register', async () => {
      ...
      expect(mockHttp.post).toHaveBeenCalledWith('/auth/register', { username: 'newuser', email: 'new@test.com', password: 'pass123' })
    })
  })

  describe('authApi.me', () => {
    it('应调用 GET /auth/me', async () => {
      mockHttp.get.mockResolvedValueOnce(mockResponse({ data: { id: 1, username: 'admin' } }))
      const { authApi } = await import('../api/auth.js')
      const result = await authApi.me()
      expect(mockHttp.get).toHaveBeenCalledWith('/auth/me', expect.objectContaining({ _silent: true }))
      expect(result.data.data.username).toBe('admin')
    })
  })

  describe('authApi.refresh', () => {
    it('应调用 POST /auth/refresh', async () => {
      mockHttp.post.mockResolvedValueOnce(mockResponse({ data: { accessToken: 'at_refreshed' } }))
      const { authApi } = await import('../api/auth.js')
      await authApi.refresh('rt_old')
      expect(mockHttp.post).toHaveBeenCalledWith('/auth/refresh', { refreshToken: 'rt_old' })
    })
  })
  ...
  describe('authApi.logout', () => {
    it('应调用 POST /auth/logout', async () => {
      ...
      expect(mockHttp.post).toHaveBeenCalledWith('/auth/logout', { refreshToken: 'rt_token' })
    })
  })
