<!--
  @file views/agent/Index.vue (V3.5.75 标准化模板)
...
async function handleEntityUpdate(form: any) {
  try {
    ...
    if (id) {
      await http.put(`${'/agent'}/${'agent'}s/${id}`, form)
      toast.success(`已更新: ${form.name || form.title}`)
    } else {
      await http.post(`${'/agent'}/${'agent'}s`, form)
...
async function handleEntityDelete(item: any) {
  try {
    const id = item.id
    if (id) {
      await http.delete(`${'/agent'}/${'agent'}s/${id}`)
    ...
    if (e?.response?.status === 404) {
      toast.warning(`后端无 ${'/agent'}/${'agent'}s API, 演示模式: ${item.name} 已标记删除`)
      entityDrawerVisible.value = false
    } else {
